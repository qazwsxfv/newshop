...
$(function(){
    ...
    $("#batchDel").click(function(){
        ...
        param.id = arr;   //{id=数组值}
        //使用异步方式，传到后台
        $.get("GoodsTypeServlet?action=batchDelId",param,function(data){
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li><span><a href="GoodsTypeServlet?action=goodsTypeToAdd"/><img src="images/t01.png" /></span>添加</a></li>
    ...
    <table class="tablelist">
        ...
        <tbody>
        		<c:forEach items="${page.list}" var="goodsType">
	       			<tr>
				        <td><input name="" type="checkbox" value="${goodsType.id}" class="single"/></td>
				        <td>${goodsType.id}</td>
				        <td>${goodsType.gtype_name}</td>
				        <td>${goodsType.gtype_parentid}</td>
				        <td>
				        	<a href="GoodsTypeServlet?action=updateInit&&id=${goodsType.id}">编辑</a>
				        	<a href="GoodsTypeServlet?action=del&&id=${goodsType.id}">删除</a>
				        </td>
			        </tr> 
        		</c:forEach>
        </tbody>
